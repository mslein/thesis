...
#GAM APPROACH
#fit GAM
gam1<-gam(yi~te(mean_temp_constant,flux_range,k=7),data=normalized) # note, lots of decisions that I blew by here
summary(gam1)
...
#plot gam surface
ggplot()+
  geom_raster(data = surf, aes(x=mean_temp_constant,y=flux_range, fill=value))+
...
full_rf_thermal<-rma.mv(yi, vi, data=dat_MA_ES, mods = 
                          ~flux_range * mean_temp_constant + stressful, 
               random = ~1 | experiment_id/ study_id/ response_id,
                 method="REML") 
full_rf_thermal
#absolute yi model 
absolute_model<-rma.mv(abs(yi), vi, data=dat_MA_ES, mods = 
                          ~flux_range * mean_temp_constant +
                        exp_age + size + org_level + resp_type, 
               random = ~1 | experiment_id/ study_id/ response_id,
                 method="REML") 
absolute_model
