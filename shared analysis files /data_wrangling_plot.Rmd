...
normalized %>%
ggplot(aes(x=mean_temp_constant, y=flux_range, fill = yi))+
  geom_jitter(data = normalized, mapping = aes(x=mean_temp_constant,y=flux_range,z= yi,fill = yi),
              alpha = 0.9, pch=21,size=3,colour="black", width = 0.7, height = 0.7)+
  scale_fill_viridis('Effect Size', limits = c(-2,5), option = "C")
```
...
```{r}
normalized %>%
  ggplot(aes(y=yi, x=mean_temp_constant, color = flux_range))+
  geom_point(size=4)+
  scale_color_viridis(option= "B")+
  theme_classic()

normalized %>%
  ggplot(aes(y=flux_range, x=mean_temp_constant, color = yi))+
  geom_jitter(size=4)+
  scale_color_viridis(option= "B")+
  theme_classic()
